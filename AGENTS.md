...
- **菜单栏应用**：程序坞没有图标，没有桌面进程表工作区。左键打开点开结束的表，表出现在菜单栏图标正下方，点外面关；右键才是开机自启 / 隐藏图标 / 打开主窗口 / 设置 / 检查更新 / 退出。禁止把菜单常挂在状态项上。禁止因拿不到图标坐标而掉到屏幕角落。隐藏菜单栏图标时必须有轻量恢复窗口（现用设置窗承载）；**进程表不能当恢复面**。
...
做成 Stats / iStat 那种只看温度没有结束的菜单栏；再加一个桌面进程表工作区或程序坞常驻图标；做成可展开进程树；做成第二个 Corral；上 Mac App Store；开沙盒；申请辅助功能 / 完整磁盘访问；用 Java；做 Windows / Linux 客户端。隐藏菜单栏图标必需的轻量恢复窗口（现用设置窗承载）不属于上述禁止范围，且进程表不能当恢复面。
...
## 2026-09-01 菜单栏专项：已落地，待 Mac 真机验收

Linux 开发机无 Xcode / Swift，源码已按全局菜单栏指南改完，**不得宣称本机已验证**。回 Mac 后：`xcodegen generate` → 构建安装 → 按下列清单验收。不读会把「源码已改」说成「已经能用」。

- 右键有开机自启（待批准是中间态、不是开，并能去登录项）、隐藏菜单栏图标、打开主窗口、设置、检查更新、退出。左键只开进程表，不弹菜单。
- 隐藏图标后，从应用程序 / Spotlight 再打开必须出示带标题栏的恢复窗（设置窗）；第二次点不能无反应。启动时若图标已隐藏也直接出示。进程表不能当恢复面。
- 恢复窗能看出仍在工作，并能开开机自启、检查更新、重新显示菜单栏图标。关最后一窗不退出。正在安装更新时允许退出。
- 菜单栏图标默认显示；隐藏状态重启后仍在。
...
- [docs/PRODUCT_CONTRACT.md](docs/PRODUCT_CONTRACT.md)：改、评审或排查菜单栏图标/模板图、隐藏图标、恢复窗口、开机自启三态或任何其他用户可见行为（人话名折行、名字列过宽、长包名省略、Chrome 显示成 `Goog...rome`、长名字减小字体、前缀省略、中间省略、整机占比、刚开机内存六十多、表头汇总、点列名排序、表出现在图标下方、掉到屏幕角落、结束边界、刷新开关、表头顶空白、结束符号悬停钉行、检查更新入口、进程表仍显示旧图标/粉底黑叉）前**必读**。不读会把 ChatGPT 拆成一堆 node、把开机六十当泄漏去改口径、把 Cursor Agent 折进 Cursor.app、把隐藏图标后的恢复面做成进程表，或把系统还在吐旧图当成图标没装进去去重做。
