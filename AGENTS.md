...
## 2026-09-04 状态栏分区点击：1.0.5 已在本机覆盖安装

v1.0.5 / 内部构建号 16 已覆盖安装进本机「应用程序」并启动，Developer ID 签名校验通过。双环与网速读数的分区点击不许再从全局事件反推位置；必须使用按钮里的透明点击层直接接收本地坐标，状态项宽度与图像一致。双环只进 CPU / 内存，读数才进网络表；网络表每次打开默认 Download 降序。发布前必须在真实菜单栏验证两个入口。
