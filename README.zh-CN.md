...
4. 点表外面关掉。右键图标可开机自启、隐藏菜单栏图标、打开主窗口、设置、检查更新或退出。
5. 开机自启默认关。隐藏菜单栏图标会打开恢复窗，可再显示图标。
...
Stats / iStat 那种只看传感器的菜单栏、程序坞图标、桌面进程表工作区、可展开进程树、Mac App Store、沙盒、辅助功能 / 完整磁盘访问、Windows / Linux 客户端。隐藏菜单栏图标后的轻量恢复窗口必须有，且不是进程表。
