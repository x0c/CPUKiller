...
- **AI 易错点**【网络名单闪烁】某一拍上下行都为 0 时不得立刻踢出该责任行 -> `NetworkListPresence` 必须对刚有过流量的行保留约 5 秒（显示 `0 KB/s`），进程已不在责任名单时才立即移除（原因：Chrome 等应用经常某一秒没包，秒级踢出再进入会让用户以为名单坏了；借鉴同类监视器的时间保持，而不是阈值迟滞）。
