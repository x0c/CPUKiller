...
- 【点击范围】双环与上、下行的点击必须由状态栏按钮内的透明点击层直接接收本地坐标；状态项宽度固定为当前图像宽度 -> 双环命中 CPU / 内存表；只有读数区域才按上、下行进入网络表。禁止从 `NSApp.currentEvent` 反推点击位置或拿状态项整块可点击留白切分（原因：前者在状态项行动回调中不能可靠地代表实际命中点，后者会把点在圆环上的点击误判为网络读数）。
