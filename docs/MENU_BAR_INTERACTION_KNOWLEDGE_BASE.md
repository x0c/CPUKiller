...
- 【点击范围】圆环和网速必须是两个独立的 `NSStatusItem`，各自使用系统按钮的单一左键动作：圆环项只直接打开 CPU / 内存表，网速项只直接打开网络表且默认 Download 降序。菜单栏按后来加入的状态项向左排：为了固定“圆环在左，网速在右”，先创建网速项再创建圆环项；不提供也不保存菜单栏左右布局切换。禁止把它们重新合成一个状态项、叠透明子视图、从 `NSApp.currentEvent` 反推坐标，或拿状态项整块留白切分（原因：菜单栏环境下同一项的局部命中顺序与坐标不可靠，已经多次把圆环错送至网络表；独立状态项从系统事件层隔离入口）。
...
- **【排错结论 2026-09-05】网速刷新抖动**：若网速状态项按当前两行实际字形计算 `NetworkLayout.width`，并在每次 `StatusItemController.renderStatusItem()` 时更新 `networkStatusItem.length`，数字位数、共同单位或上下行大小字号一变化，系统就会重新排版整块状态项，用户会看到刷新瞬间左右抖动。修复必须固定网速排版框、单位右边线和最右箭头列；数字只在左侧伸缩，短读数的空位只能留在数字列左侧，并且只在状态项重建时设置固定长度，刷新时只替换图像内容。禁止为了省下少量空白恢复按当前字形动态改变外框宽度，也禁止交换方向、删掉单位或箭头来掩盖抖动。
