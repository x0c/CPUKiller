...
- **【排错 2026-09-05】菜单栏浮层截图**：进程表 / 网络表浮层挂在状态栏窗口层级（layer ≠ 0）。`macos-window-screenshot` 的 `winshot.py list/capture` 会故意丢掉这类窗口，表现为“应用在跑、列表也点开了，但 list 里没有 CPU Killer”。核对浮层字号、颜色或布局时，先用辅助功能点开对应状态项，再用 `screencapture -l<窗口ID>` 按 Quartz 窗口编号截图；不要据此误判浮层没显示，也不要为了迁就截图工具去改浮层窗口层级。
